...
class QLearner:
    def __init__(self, mac, scheme, logger, args):
        ...
        self.mac = mac
        self.logger = logger
        if args.learner == "q_learner":  # 兼容子类继承，避免调用基类内部语句引起异常
            if args.name == "hetero_qmix":
                agent_params, med_params = mac.parameters()
                self.params = list(agent_params) + list(med_params)
            else:
    ...
    def train(self, batch: EpisodeBatch, t_env: int, episode_num: int):
        ...
        assert torch.isnan(mac_out).int().sum() == 0, print('-----MAC NAN!-----', torch.isnan(mac_out), mac_out, mac_out.shape)
        ...
        if self.mixer is not None:
            chosen_action_qvals = self.mixer(chosen_action_qvals, batch["state"][:, :-1])  # torch.Size([32, 70, 1])
        ...
        if (episode_num - self.last_target_update_episode) / self.args.target_update_interval >= 1.0:
            ...
            if self.args.lr_episode_size > 0 and self.args.adam_lr > 0:
                print('-----' * 5)
                print('LR_scheduler Working! Adam_lr=%.7f' % self.lr_scheduler.get_lr()[0])
                print('-----' * 5)
